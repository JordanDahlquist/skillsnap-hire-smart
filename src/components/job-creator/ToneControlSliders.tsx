  ...
  return <Card className="mb-4">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          {onRegenerate && <Button variant="outline" size="sm" onClick={onRegenerate} disabled={isGenerating} className="text-xs h-8 px-3">
      ...
      <CardContent className="space-y-2">
        <div className="grid grid-cols-3 gap-4">
          {toneSettings.map(setting => <div key={setting.key} className="space-y-1">
              <TooltipProvider>
                <Tooltip>
              ...
              <div className="px-1">
                <Slider value={[setting.value]} onValueChange={values => onToneChange(setting.key, values[0])} min={1} max={5} step={1} className="w-full" />
