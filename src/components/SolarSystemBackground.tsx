...
export const SolarSystemBackground = () => {
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden">
      ...
      {/* Enhanced solar system container */}
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative w-[900px] h-[900px] opacity-40">
          
          {/* Central sun with intense glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            {/* Outer glow rings */}
            <div className="absolute w-16 h-16 bg-blue-400/20 rounded-full -translate-x-1/2 -translate-y-1/2 animate-pulse" 
                 style={{ animationDuration: '3s' }} />
            <div className="absolute w-12 h-12 bg-blue-300/30 rounded-full -translate-x-1/2 -translate-y-1/2 animate-pulse" 
            ...
            <div className="absolute w-8 h-8 bg-gradient-to-br from-blue-300 via-blue-400 to-blue-600 rounded-full -translate-x-1/2 -translate-y-1/2 animate-pulse shadow-lg" 
                 style={{ 
                   boxShadow: '0 0 20px 8px rgba(59, 130, 246, 0.6), 0 0 40px 12px rgba(59, 130, 246, 0.3)',
                   animationDuration: '1.5s'
                 }} />
          </div>
          
          {/* Enhanced orbit rings with asteroid belts */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 border border-blue-300/15 rounded-full animate-spin-slow">
            {/* Asteroid particles */}
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={`asteroid-1-${i}`}
                className="absolute w-0.5 h-0.5 bg-gray-400 rounded-full opacity-60"
                style={{
                  left: '50%',
                  top: '50%',
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-60 h-60 border border-blue-300/12 rounded-full animate-spin-slower">
            {/* Asteroid particles */}
            {Array.from({ length: 12 }).map((_, i) => (
              <div
                key={`asteroid-2-${i}`}
                className="absolute w-0.5 h-0.5 bg-gray-300 rounded-full opacity-40"
                style={{
                  left: '50%',
                  top: '50%',
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 border border-blue-300/8 rounded-full animate-spin-slowest">
            {/* Asteroid particles */}
            {Array.from({ length: 16 }).map((_, i) => (
              <div
                key={`asteroid-3-${i}`}
                className="absolute w-0.5 h-0.5 bg-gray-200 rounded-full opacity-30"
                style={{
                  left: '50%',
                  top: '50%',
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 animate-spin-slowest">
            ...
            <div className="absolute right-0 top-1/2 translate-x-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-gradient-to-br from-slate-200 to-slate-400 rounded-full shadow-md" 
                 style={{ boxShadow: '0 0 8px rgba(203, 213, 225, 0.6)' }} />
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 30 }).map((_, i) => (
          <div
            key={`dust-${i}`}
            className="absolute w-0.5 h-0.5 bg-gray-400/30 rounded-full animate-float"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
